...
class Database:
    ...
    async def update_balance(self, user_id: int, amount: int):
        ...
        await asyncio.get_event_loop().run_in_executor(None, _update_balance)
    ...
    async def update_bank_balance(self, user_id: int, amount: int):
        ...
        await asyncio.get_event_loop().run_in_executor(None, _update_bank_balance)
    ...
    async def set_profile_closed(self, user_id: int, closed: bool):
        """Установка статуса закрытия профиля"""
        def _set_profile_closed():
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
        ...
        await asyncio.get_event_loop().run_in_executor(None, _set_profile_closed)
    ...
    async def ban_user(self, user_id: int):
        ...
        await asyncio.get_event_loop().run_in_executor(None, _ban_user)
    ...
    async def unban_user(self, user_id: int):
        ...
        await asyncio.get_event_loop().run_in_executor(None, _unban_user)
    ...
    async def is_user_banned(self, user_id: int) -> bool:
        ...
        return await asyncio.get_event_loop().run_in_executor(None, _is_user_banned)
    ...
    async def get_top_users(self, limit: int = 10) -> List[Dict]:
        ...
        def _get_top_users():
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM users WHERE is_banned = FALSE ORDER BY balance DESC LIMIT ?", (limit,))
    ...
    async def get_top_banks(self, limit: int = 10) -> List[Dict]:
        ...
        def _get_top_banks():
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM users WHERE is_banned = FALSE ORDER BY bank_balance DESC LIMIT ?", (limit,))
            rows = cursor.fetchall()
            conn.close()
    ...
    async def get_leaderboard(self, limit: int = 5) -> List[Dict]:
        """Получение лидерборда по дневным выигрышам"""

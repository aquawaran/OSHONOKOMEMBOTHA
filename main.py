...
async def handle_balance(message: Message):
    user = await db.get_user(message.from_user.id)
    if user:
        await message.reply(f"💰 Ваш баланс: {user['balance']} MEM", reply_to_message_id=message.message_id)

async def handle_bank(message: Message):
    user = await db.get_user(message.from_user.id)
    if user:
        await message.reply(f"🏦 Баланс банка: {user['bank_balance']} MEM", reply_to_message_id=message.message_id)
...
async def handle_profile(message: Message):
    ...
    if target_id != message.from_user.id and user.get('profile_closed', False):
        await message.reply("🔒 Профиль этого пользователя закрыт", reply_to_message_id=message.message_id)
        return
    ...
    if user.get('avatar_path'):
        try:
            await message.reply_photo(user['avatar_path'], caption=f"<blockquote>{profile_text}</blockquote>", parse_mode="HTML", reply_to_message_id=message.message_id)
            return
        except:
            pass  # Если не удалось отправить фото, продолжаем без аватарки
    
    await message.reply(f"<blockquote>{profile_text}</blockquote>", parse_mode="HTML", reply_to_message_id=message.message_id)

async def handle_inventory(message: Message):
    await message.reply("🎒 Ваш инвентарь:\n\nПусто (скоро появятся предметы)", reply_to_message_id=message.message_id)
...
async def handle_bonus(message: Message):
    ...
    if user_id in bonus_cooldowns:
        ...
        if hours_passed < BONUS_COOLDOWN_HOURS:
            ...
            await message.reply(f"⏳ Бонус можно получить через {int(remaining_hours)} часов", reply_to_message_id=message.message_id)
            return
    ...
    try:
        ...
        if channel_member.status == "left":
            ...
            await message.reply("❌ Для получения бонуса нужно подписаться на канал @InitialNFT", reply_markup=keyboard, reply_to_message_id=message.message_id)
            return
    except:
    ...
    await message.reply(f"🎁 Бонус получен!\n💰 +{bonus} MEM\n\nБонус можно получить раз в 24 часа", reply_to_message_id=message.message_id)
...
async def handle_give(message: Message):
    try:
        ...
        if len(parts) < 2:
            await message.reply("❌ Использование: дать [сумма]", reply_to_message_id=message.message_id)
            return
        ...
        if amount <= 0:
            await message.reply("❌ Сумма должна быть положительной", reply_to_message_id=message.message_id)
            return
        
        if sender_user['balance'] < amount:
            await message.reply("❌ Недостаточно MEM", reply_to_message_id=message.message_id)
            return
        
        target_id = await get_target_user(message)
        
        if not target_id:
            await message.reply("❌ Ответьте на сообщение пользователя", reply_to_message_id=message.message_id)
            return
        
        # Проверяем что не пытаются перевести боту
        if target_id == message.bot.id:
            await message.reply("❌ Нельзя переводить MEM боту", reply_to_message_id=message.message_id)
            return
        
        if target_id == message.from_user.id:
            await message.reply("❌ Нельзя перевести самому себе", reply_to_message_id=message.message_id)
            return
        ...
        await message.reply(f"✅ Переведено {amount} MEM", reply_to_message_id=message.message_id)
    except ValueError:
        await message.reply("❌ Неверная сумма", reply_to_message_id=message.message_id)
